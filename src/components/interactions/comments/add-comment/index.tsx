...
const AddComment = ({ play, setReload }: CommentProps) => {
    ...
    if (data) {
      ...
      if (mentionedUserIds.length > 0) {
        ...
        const recipients = mentionedUserIds.filter(
          (id) => id !== user.userId && id !== play.author.id,
        );
      ...
      if (play.author.email !== user.email && play.author.send_notifications) {
        await sendEmail({
          video: play.video,
          comment: data,
          play: play.play,
  ...
  return (
    ...
    >
      <MentionInput
        entityType="comment"
        teamId={play.play.exclusive_to} // Pass the team ID to fetch relevant users
        value={commentText}
